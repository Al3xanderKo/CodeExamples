...
//		assert ciastka > 0 : "Čiastka musí byť vačšia ako 0 ! "; 
	...
	public void vlozNaUcet(double ciastka, Object objekt) {
		aktualnyZostatok -= ciastka;
		if (objekt instanceof BeznyUcet) {
			((BeznyUcet) objekt).vloz(ciastka);
		}
		if (objekt instanceof TerminovanyUcet) {
			((TerminovanyUcet) objekt).vloz(ciastka);
		}
